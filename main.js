...
kryptoCurrency = new Blockchain(config.name, config.miningDifficulty, config.blockReward, config.halvingEvent)
...
if (config.audit === true){
    for (block in kryptoCurrency.blockArray){
        for (transaction in kryptoCurrency.blockArray[block].transactions){
            console.log(JSON.stringify(kryptoCurrency.blockArray[block].transactions[transaction], null, 4))
            console.log(JSON.stringify(kryptoCurrency.blockArray[block].transactions[transaction].isValid(), null, 4))
        }
    }
    // single check all transactions
    for(block in kryptoCurrency.blockArray){
        for(transactions in kryptoCurrency.blockArray[block].transactions){
            console.log(`Block ${block}; Transaction; ${transactions}; ${JSON.stringify(kryptoCurrency.blockArray[block].transactions[transactions].isValid(), null, 4)}`)
        }
    }
}
